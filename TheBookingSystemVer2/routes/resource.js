...
const resourceSchema = z.object({
  resourceName: z.string()
    ...
    .regex(
      /^[a-zA-Z0-9_]+$/,
      "Resource name can only contain alphanumeric characters and underscores",
    ),
...
export async function getResources(req) {
  const query =
    `SELECT resource_id, resource_name, resource_description FROM zephyr_resources`;
  try {
...
export async function registerResource(req) {
  ...
  try {
    ...
    const query =
      `INSERT INTO zephyr_resources (resource_name, resource_description) VALUES ($1, $2)`;
